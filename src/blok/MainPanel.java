...
public class MainPanel extends javax.swing.JPanel implements MouseListener, KeyListener {
    ...
    public void paintComponent(Graphics g) {
        ...
        g2d.drawImage(cenario.createPlanoDeFundo().carregar(), 0, 0, null);
        g2d.drawImage(cenario.createChao().carregar(), size.width/2-450, size.height/2-10+260, null);

        for (Rectangle rect : m_bodyRect.values()) {
            if (rect != m_player) {
                // Block
                TexturePaint texturePaint = new TexturePaint(cenario.createBloco().carregar(), rect);
